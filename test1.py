...
def upload_image():
    ...
    if food_id is None:
        # Nếu món ăn chưa có trong database, thêm món ăn mới vào bảng `food`
        try:
            query_insert_food = "INSERT INTO food (name) VALUES (%s)"
            cursor.execute(query_insert_food, (food_name.title(),))
            conn.commit()
            # Lấy ID của món ăn mới thêm
            food_id = cursor.lastrowid
        except mysql.connector.errors.IntegrityError as e:
            # Nếu xảy ra lỗi trùng lặp, trả về thông báo lỗi
            return jsonify({'error': f'Duplicate entry: {food_name} already exists.'}), 400
    ...
    if file:
        ...
        query_insert_image = """
        INSERT INTO food_imgs (food_name, label, description, img_path, feature) 
        VALUES (%s, %s, %s, %s, %s)
        """
        cursor.execute(query_insert_image, (food_name, food_id, description, final_path, feature_blob))
        conn.commit()
...
def get_food():
    ...
    cursor.execute("SELECT food_name, img_path, description FROM food_imgs")
...
def get_random_food_images():
    all_foods = get_food()  # Lấy tất cả món ăn từ cơ sở dữ liệu
    print(all_foods)  # In ra dữ liệu món ăn
    random_foods = random.sample(all_foods, 50) if len(all_foods) > 50 else all_foods

    # Tạo danh sách các thông tin món ăn
    foodName = [food[0] for food in random_foods]
